...
- Red flags: restaurant table booking required inside the PMS or POS (Mews has NO table-booking capability anywhere — not in the PMS, not in Mews POS, not in the Booking Engine; refer to a specialist Marketplace integration such as SevenRooms, OpenTable, Resy or Tock via the Open API); rate mixing within a single reservation; external activity packages with no partner API; complex child pricing with many age bands; 5★ ultra-luxury with bespoke concierge/CRM expectations; residential/semi-residential flows.
- Fastest DQ: "Do you currently use a dedicated restaurant table-booking system (SevenRooms / OpenTable / Resy / Tock), and is keeping it independent of the PMS acceptable?" — if they expect table booking to live inside the PMS or POS, that's a hard no for Mews.
...
- **Restaurant table booking / table management.** Not in PMS, not in Mews POS, not in the Booking Engine. For F&B-led or restaurant-heavy properties, point to a specialist via the Open API (SevenRooms, OpenTable, Resy, Tock). Do NOT phrase any Mews POS pitch as "manage tables" — Mews POS handles the unified F&B folio, charges, splits and tax, but it is not a table-management system.
