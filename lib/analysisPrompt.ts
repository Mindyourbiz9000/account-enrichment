...
- **\`guest_reviews\`**: Requires ≥2 verbatim quotes pulled from the dossier's \`reputation.positive_themes\` or \`reputation.negative_themes\` quote arrays. Use the exact quote text. If fewer than 2 quotes exist for a candidate challenge, switch evidence_type or drop it. Set \`payment_related: true\` if the Payments radar above matches.
  ...
  **MANDATORY — no prose summaries**: when \`evidence_type\` is \`guest_reviews\`, you MUST populate \`quotes[]\` with the actual verbatim quote objects (copied from the dossier's reputation arrays). Writing a sentence like "3 reviews mention long queues" or "Multiple guests reported slow check-in" in \`evidence_basis\` is STRICTLY FORBIDDEN — that is a prose summary, not evidence. The verbatim quote text goes in \`quotes[].text\`, not in \`evidence_basis\`. Omit \`evidence_basis\` entirely when \`evidence_type\` is \`guest_reviews\`.
