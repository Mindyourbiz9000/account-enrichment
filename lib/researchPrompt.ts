...
1. Research coverage areas — work through ALL of the following before returning. You are not counting searches; you are ensuring complete coverage of each topic area. Do NOT spend any research effort on Mews itself — the playbook above already tells you everything Mews does.
   ...
   - [ ] **Live room rates & pricing.** Open the hotel's listing on Booking.com (primary), then Expedia / Hotels.com / Google Hotels / the hotel's own booking engine as fallbacks. Check a midweek night 3–4 weeks out and the nearest weekend night. Record the cheapest standard double rate for each, the source name, and the date checked. Do not estimate rates when an OTA listing exists. Add the source URL to the \`sources\` array.
   ...
   - [ ] **Tech-stack and commercial signals — tiered confidence.** Gather evidence at three levels and tag every \`tech_stack_signals\` entry accordingly. **HIGH**: booking-engine footer ("Powered by X"), Mews Marketplace / SiteMinder / Duetto integration-directory listings, brand tech pages, recent press. **MEDIUM**: job postings ≤24 months old naming a PMS/POS/RMS or payment gateway, guest reviews naming a system by name, directory listings ≤18 months old. **LOW**: system inferred from segment/brand/market with no direct evidence — tag it explicitly as "inferred". Also check GDS codes and loyalty partners.
   ...
   - [ ] **Ownership, investment & capex signals.** Search for the hotel's parent company, asset manager, or private equity backer. Check hospitality trade press (Skift, Hotel Management, Hospitality Net) and the brand's own news page for acquisitions, ownership changes, announced renovations, capex projects, or rebrands. These signals predict upcoming PMS replacement decisions — emit in \`reputation.recent_press\` and as a \`press_or_ownership\` challenge when found.
   - [ ] **Distribution strategy & OTA dependency.** Check for a "Best Rate Guarantee" or direct booking incentive on the hotel's website. Identify the booking engine vendor (look for a "Powered by" badge or footer hint). Search job ads for "channel manager", "OTA strategy", "Expedia", "Booking.com" — heavy OTA language signals VCC reconciliation pain; flag in \`tech_stack_signals\` and create a \`payment_related: true\` challenge where applicable.
   - [ ] **Staff & culture signals.** Search Glassdoor and Indeed for staff reviews of this hotel. Phrases like "outdated system", "manual work", "spreadsheets", "understaffed front desk" indicate operational pain directly relevant to a Mews pitch. Read full job postings — any named PMS, POS, RMS, or standalone payment gateway (Stripe, Adyen, Worldline) is the strongest tech-stack evidence available.
   - [ ] **Local market context.** Briefly characterise: demand mix (leisure / corporate / MICE / transit), key seasonal peaks/troughs, and at least 1–2 nearby demand generators (convention centre, airport, sports venue, university, major annual event). Emit in \`property_profile.local_market_context\`. Reference in discovery questions — it makes the pitch locally grounded.
...
2b. **Cross-verify critical facts before emitting.** For the three fields below, require at least 2 independent sources or flag explicitly as "single-source only, verify before call":
   - **GM name**: hotel "Team / About / Leadership / Contact" page AND (LinkedIn showing the hotel as current employer OR a second press mention OR Google Business profile).
   - **Room count**: hotel's own website AND (Booking.com / Expedia listing OR a credible press article). If they disagree, emit the more recent figure and note the discrepancy.
   - **Star rating**: hotel's own website AND (official classification body OR Booking.com / Google / TripAdvisor). If two sources disagree, emit both and flag the discrepancy.
...
4. **Live room rates — look them up, do not estimate.** Real rates are publicly available — always search before falling back to estimation:
   a. **Search**: look up the hotel on Booking.com by name + city. Alternatively use Google Hotels, Expedia, or the hotel's own direct booking engine ("Book Now" button).
   b. **Dates**: check a midweek night (Tue or Wed check-in) 3–4 weeks from today for a representative rack rate, then the nearest Friday or Saturday night for the weekend rate.
   c. **Room type**: cheapest available standard double or twin (no breakfast bundle, no loyalty rate, no package). Do not use a suite.
   d. **Record as**: \`"from €X/night midweek, from €Y/night weekend (standard double, Booking.com, [date checked])"\` — put this in \`property_profile.price_range\` and mirror it in \`estimated_adr\`.
   e. **Source priority**: (1) Booking.com, (2) Expedia / Hotels.com, (3) Google Hotels, (4) the hotel's own booking engine. Add the source URL to \`sources\`.
   f. **Only if genuinely unavailable** (property closed, no OTA listing after checking all four sources): fall back to a segment-based estimate, label it \`"est."\`, and explain the basis (e.g. "est. — no OTA listing found; comparable 4★ urban boutiques in this market typically €180–220/night midweek").
   g. **Occupancy**: no live public feed exists — use observable signals (review volume vs. room count, RevPAR commentary in press, STR/CoStar references if surfaced). Express as a qualitative note in \`occupancy_notes\`, never a bare percentage without a cited source.
...
6c. **Contacts are MANDATORY. The \`contacts\` array must NEVER be empty.**
    - Tier 1 (preferred): at least 2 named individuals — GM is the bare minimum, plus one commercial role (DOSM / Director of Sales / Revenue Manager / F&B Director / Events Manager / Owner / Asset Manager). Source priority: (1) hotel website "Team / About / Leadership / Contact" page (most authoritative — set \`tenure_confidence: "current"\` if confirmed here), (2) brand press archive, (3) PR Newswire / Hospitality Net / Hotel Management announcements (use the article date as \`source_date\`), (4) LinkedIn (use only as confirmation — LinkedIn alone without a hotel-page match = \`tenure_confidence: "unverified"\`).
    - **STALENESS RULE**: if the only evidence for a person's current role is >12 months old, set \`tenure_confidence: "stale_risk"\`. If two sources conflict (e.g. hotel website shows one GM, LinkedIn shows another), emit both with \`tenure_confidence: "conflicting_sources"\` and note the discrepancy. Always cross-check LinkedIn against the hotel's own website — if the LinkedIn profile does NOT show this hotel as the current employer, do not emit that contact.
    - **EMAIL 95% rule (tightened)**: only include an email if it appears on a public page dated/crawled within 18 months AND at the hotel's own or brand domain. Pattern-guessed emails (firstname.lastname@domain) are allowed ONLY if the naming convention is confirmed by at least one other verified email at the same domain — mark as \`email_confidence: "guessed"\`. Never use data-broker or stale cached pages as the sole email source.
    - **LINKEDIN 95% rule (tightened)**: only emit a LinkedIn URL you actually navigated to and read, and only if the profile shows this hotel as the current employer. Never construct a LinkedIn URL by guessing from a name.
    - Tier 2 (fallback when Tier 1 truly yields nothing): pull \`reservations@…\` / \`sales@…\` / \`info@…\` / \`gm@…\` email + reception phone from the hotel's contact page, and emit them as a contact with \`role: "Reservations Team"\` (or "Sales Team" / "Reception") and a \`source\` pointing at the contact page URL. This row MUST appear if no named person was findable.
    - Never return an empty contacts array. If you genuinely cannot find either a name OR a shared inbox after a real effort, the contacts research step wasn't done properly — go back and look again.
...
- \`contacts\`: **NEVER EMPTY.** Ideally 2+ named individuals (GM + one commercial role), each with \`tenure_confidence\` and \`source_date\` set. Contacts with \`tenure_confidence: "stale_risk"\` — did you try to find a more recent source? Did you fall back to \`reservations@\` / \`sales@\` / \`info@\` as "Reservations Team" if no named person was findable?
- \`tech_stack_signals\`: at least 1–2 observed systems, every entry has a \`confidence\` level set (high / medium / low). No entry marked "high" without a direct public URL. No "medium" from job posts >24 months old. Inferred entries tagged as "low"?
...
- \`property_profile.local_market_context\`: demand-mix + seasonality note + at least 1 named demand generator present?
- Ownership/investment signals checked in trade press? Any signal emitted in \`recent_press\` or as a \`press_or_ownership\` challenge?
- Distribution/OTA dependency: booking engine vendor identified (or noted as unconfirmed)? Heavy OTA reliance flagged in \`tech_stack_signals\` and linked to a \`payment_related: true\` challenge where applicable?
