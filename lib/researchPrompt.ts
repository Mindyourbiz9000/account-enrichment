...
3. Never fabricate contacts, emails, LinkedIn URLs or hotel data. For contact emails and LinkedIn URLs: only emit a field if the exact string appears on a public page (hotel website, press release, conference bio, LinkedIn profile currently showing this hotel as employer). Set email_confidence to "verified" only when you found the exact address printed on a page you visited. Do NOT construct or derive email addresses from naming patterns (firstname.lastname@domain) — a constructed email is an invented fact, not a sourced one, even if the pattern is common. Do NOT emit a "guessed" email. Never invent a source URL that you didn't actually visit.
4. For ADR, follow the mandatory live lookup procedure in Rule 1 (Booking.com primary, midweek standard double, 3–4 weeks out). If the lookup yields a real rate, emit it with the source and date. If no OTA listing exists after a genuine effort, omit \`estimated_adr\` entirely — do NOT estimate, invent, or construct a plausible range. A missing field is correct; a made-up number is not.
5. PRIMARY DELIVERABLE: \`mews_qualification\`, \`mews_positioning\`, and \`key_challenges\` — spend the bulk of your output budget there. Ground every mews_angle, opening_hook, value-prop and discovery question in the primer above.
   **Reputation rules (positive_themes / negative_themes):**
   ...
   - Emit ALL verbatim quotes you found that back the theme (minimum 2, no upper cap). Never paraphrase. Never pick-and-choose — include every quote you actually retrieved from a review platform.
